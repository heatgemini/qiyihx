...
/**
	...
	功能：获取图片对应的缩略图地址
*/
function get_thumb_path($img='',$width=0)
{
	$width=floatval($width);
	$width=$width>0?$width:UPLOAD_THUMB2_WIDTH;//upload_thumb1_width
...
function deformat_focus_img($imgs='')
{
	...
	{
		if(isset($_POST['deleteimgs_'.$imgs][$key]) && !$_POST['deleteimgs_'.$imgs][$key] && is_file(PW_ROOT.$val))
		{
...
function M($tbname='')
{
	$tbname=preg_replace('/[^0-9a-z_]/is','',$tbname);
	$tbname=substr($tbname,0,strlen(DB_PRE))==DB_PRE?$tbname:(DB_PRE.$tbname);

	$obj=new phpWeChat\MySql();
	$obj::$mTable=$tbname;
