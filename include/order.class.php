...

{
	...
	*/
	...
	public static function orderList($status,$out_trade_no,$pagesize=10,$mod='',$action='',$sellerid=0) 

	{
		...
		$where.=$sellerid?' AND `sellerid`='.$sellerid:'';
	...
	public static function orderNoLimitList($userid=0,$status=0,$mod='') 

	{
		...
		{
			return MySql::fetchAll("SELECT * FROM `".DB_PRE.self::$mOrderTable."` WHERE `userid`=".intval($userid)." AND `mod`='".str_callback_w($mod)."' AND `status`=".intval($status)." ORDER BY `id` DESC");
		...
		{
			return MySql::fetchAll("SELECT * FROM `".DB_PRE.self::$mOrderTable."` WHERE `userid`=".intval($userid)." AND `mod`='".str_callback_w($mod)."' ORDER BY `id` DESC");
	...
	public static function userOrderCount($userid,$status=0,$mod='')
	{
...

		{
			...
			if($mod)
			{
				return get_cache_counts("SELECT COUNT(*) AS count FROM `".DB_PRE.self::$mOrderTable."` WHERE `mod`='".str_callback_w($mod)."' AND `userid`=".intval($userid));
			}
			else
			{
		...
		if($mod)
		{
			return get_cache_counts("SELECT COUNT(*) AS count FROM `".DB_PRE.self::$mOrderTable."` WHERE `mod`='".str_callback_w($mod)."' AND `userid`=".intval($userid)." AND `status`=".intval($status)); 
		}
		else
		{
