<?php
// +----------------------------------------------------------------------
// | phpWeChat 银行卡操作类 Last modified 2016/7/21 16:56
// +----------------------------------------------------------------------
...
class BankCard
{
	public static function getCardBank($cardno='')
	{
		$bankname='';

		$cardno=preg_replace('/[^0-9]/i','',$cardno);

		$cardno = substr($cardno, 0, 8); 
		if (isset(self::$mBankList[$cardno])) 
		{ 
			$bankname = self::$mBankList[$cardno];
		} 

		$cardno = substr($cardno, 0, 6); 
		if (isset(self::$mBankList[$cardno])) 
		{ 
			$bankname = self::$mBankList[$cardno]; 
		} 

		$cardno = substr($cardno, 0, 5); 
		if (isset(self::$mBankList[$cardno])) 
		{ 
			$bankname = self::$mBankList[$cardno]; 
		} 

		$cardno = substr($cardno, 0, 4); 
		if (isset(self::$mBankList[$cardno])) 
		{ 
			$bankname = self::$mBankList[$cardno]; 
		} 
		...
		unset($bankname[1]);
		return str_replace('贷记卡','信用卡',implode('-',$bankname));
	}

	private static $mBankList=array(
	'621098' => '邮储银行-绿卡通-借记卡',
