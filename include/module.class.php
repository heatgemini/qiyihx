...
*/
	...
	public static function ucModuleList($isuc=array(1))
	{
		//echo "SELECT * FROM `".DB_PRE.self::$mModTable."` WHERE `isuc` IN(".implode($isuc).") AND `issystem`=0 AND `disabled`=0 ORDER BY `orderby` ASC";
		return MySql::fetchAll("SELECT * FROM `".DB_PRE.self::$mModTable."` WHERE `isuc` IN(".implode(',',$isuc).") AND `issystem`=0 AND `disabled`=0 ORDER BY `orderby` ASC");
	}

	public static function menuModuleList($parentkey='0')
	{
		if($parentkey=='dcb26674c6706093' || $parentkey=='system')
		{
			return MySql::fetchAll("SELECT * FROM `".DB_PRE.self::$mModTable."` WHERE `parentkey`='".str_callback_w($parentkey)."' AND `issystem`=0 AND `disabled`=0  ORDER BY `orderby` ASC");
		}
		else
		{
			//echo "SELECT * FROM `".DB_PRE.self::$mModTable."` WHERE `issystem`=0 AND `disabled`=0 AND `key`!='dcb26674c6706093' AND (`parentkey`='".str_callback_w($parentkey)."' OR `ispch5`=1)   ORDER BY `orderby` ASC";
			return MySql::fetchAll("SELECT * FROM `".DB_PRE.self::$mModTable."` WHERE `issystem`=0 AND `disabled`=0 AND `key`!='dcb26674c6706093' AND (`parentkey`='".str_callback_w($parentkey)."' OR `ispch5`=1)   ORDER BY `orderby` ASC");
		}
