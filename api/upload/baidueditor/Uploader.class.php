...
class Uploader
{
    ...
    private function upFile( $base64 )
    {
        ...
        if ( $this->stateInfo == $this->stateMap[ 0 ] ) {
            if ( !move_uploaded_file( $file[ "tmp_name" ] , PW_ROOT.$this->fullName ) ) {
                $this->stateInfo = $this->getStateInfo( "MOVE" );
            }
			else
			{
				setwatermark(PW_ROOT.$this->fullName);
			}
        }
    }
    ...
    {
		//生成缩略图 2016-04-02 15:05
		Upload::createImageThumb(PW_ROOT.$this->fullName,600);
		Upload::createImageThumb(PW_ROOT.$this->fullName,200);
        return array(
    ...
    private function getFolder()
    {
        ...
        $pathStr .= date( "Y-m-d" );
        if ( !file_exists( PW_ROOT.$pathStr ) ) {
            if ( !mkdir( PW_ROOT.$pathStr , 0777 , true ) ) {
                return false;
            }
        }
        return $pathStr;
